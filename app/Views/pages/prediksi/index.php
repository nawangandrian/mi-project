...
<style>
    ...
    .pred-model-strip {
        ...
        gap: 12px;
        padding: 12px 18px;
        background: linear-gradient(135deg, rgba(16, 183, 127, 0.07), rgba(0, 151, 184, 0.04));
        border: 1px solid rgba(16, 183, 127, 0.22);
    ...
    .pred-strip-dot {
        ...
        box-shadow: 0 0 0 3px rgba(16, 183, 127, 0.2);
    ...
    @keyframes pDot {
        ...
        50% {
            box-shadow: 0 0 0 6px rgba(16, 183, 127, 0.08)
        }
    }
    ...
    .pred-card-icon {
        width: 38px;
        height: 38px;
        background: linear-gradient(135deg, rgba(74, 159, 212, 0.15), rgba(0, 151, 184, 0.1));
        border: 1px solid rgba(74, 159, 212, 0.2);
        border-radius: 10px;
        display: flex;
        align-items: center;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function goPage(page) {
            ...
            // Scroll ke toolbar
            document.getElementById('searchPrediksi')?.scrollIntoView({
                behavior: 'smooth',
                block: 'nearest'
            });
        }
        ...
        function applySort(val) {
            const sortFn = (a, b) => {
                const aQty = parseFloat(a.dataset.qty),
                    bQty = parseFloat(b.dataset.qty);
                const aNama = a.dataset.nama,
                    bNama = b.dataset.nama;
                switch (val) {
                    case 'qty_desc':
                        return bQty - aQty;
                    case 'qty_asc':
                    ...
                    case 'nama_desc':
                        return bNama.localeCompare(aNama);
                    default:
                        return 0;
                }
            };
        ...
        if (ctx) {
            const chartData = <?= json_encode(
                                    array_map(
                                        ...
                                        array_slice(
                                            usort($prediksiList, fn($a, $b) => (float)$b['qty_prediksi'] <=> (float)$a['qty_prediksi'])
            ...
            const labels = chartData.map(d => {
                const p = d.nama.split(' ');
                return p.length > 3 ? p.slice(0, 3).join(' ') + '…' : d.nama;
            });
            const predVals = chartData.map(d => d.prediksi);
            const datasets = [{
                label: 'Prediksi',
                data: predVals,
                backgroundColor: 'rgba(74,159,212,0.75)',
                borderColor: 'rgba(74,159,212,1)',
                borderWidth: 1,
                borderRadius: 5,
                borderSkipped: false
            }];
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            padding: 12,
                            callbacks: {
                                label: c => ` ${c.dataset.label}: ${c.parsed.y !== null ? c.parsed.y.toFixed(1) : '—'} unit`
                            }
                        }
                    },
                    scales: {
