...
<div class="content-wrapper-inner">
    ...
    $terverifikasi    = array_filter($prediksiList, fn($r) => $r['qty_aktual'] !== null);
    $totalTerverif    = count($terverifikasi);

    $mapeGlobal = null;
    if (!empty($terverifikasi)) {
        $mapeSum = 0; $mapeCount = 0;
        foreach ($terverifikasi as $r) {
            if ((float)$r['qty_aktual'] > 0) {
                $mapeSum += abs(((float)$r['qty_aktual'] - (float)$r['qty_prediksi']) / (float)$r['qty_aktual']) * 100;
                $mapeCount++;
            }
        }
        if ($mapeCount > 0) $mapeGlobal = $mapeSum / $mapeCount;
    }
    $akurasiGlobal = $mapeGlobal !== null ? max(0, 100 - $mapeGlobal) : null;
    ?>
    <div class="rw-stats-row">
        ...
        <div class="rw-stat-card">
            <div class="rw-stat-icon" style="background:rgba(212,160,23,0.12);color:var(--accent-yellow)">
                <i class="bi bi-patch-check-fill"></i>
            </div>
            <div class="rw-stat-val"><?= $totalTerverif ?></div>
            <div class="rw-stat-lbl">Sudah Terverifikasi</div>
        </div>
        <?php if ($akurasiGlobal !== null): ?>
        <div class="rw-stat-card highlight-stat">
            <div class="rw-stat-icon" style="background:rgba(16,183,127,0.12);color:var(--accent-green)">
                <i class="bi bi-bullseye"></i>
            </div>
            <div class="rw-stat-val" style="color:<?= $akurasiGlobal >= 80 ? 'var(--accent-green)' : ($akurasiGlobal >= 60 ? 'var(--accent-yellow)' : 'var(--accent-red)') ?>">
                <?= number_format($akurasiGlobal, 1) ?>%
            </div>
            <div class="rw-stat-lbl">Akurasi Keseluruhan</div>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="card-mg">
        <div class="mg-card-header">
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                ...
                <button class="btn-mg btn-outline-mg btn-sm-rw btn-sinkron-aktual"
                        data-tahun="<?= (int)$periodeAktif['tahun'] ?>"
                        data-bulan="<?= (int)$periodeAktif['bulan'] ?>"
                        id="btnSinkronAktifPeriode"
                        style="gap:5px">
            ...
            <div class="mg-card-toolbar">
                ...
                <div class="rw-sort-wrap">
                    <select id="rwSort" class="form-mg form-mg-sm">
                        ...
                        <option value="mape_asc">MAPE ↑ Terbaik</option>
                    </select>
                </div>
            ...
            <table class="table-mg rw-table" id="rwTable">
                <thead>
                    <tr>
                        ...
                        <th style="text-align:right;width:120px">Qty Aktual</th>
                        <th style="text-align:right;width:110px">Selisih</th>
                        <th style="text-align:center;width:95px">MAPE</th>
                ...
                <?php foreach ($prediksiList as $i => $p):
                    ...
                    $selisih = isset($p['selisih']) && $p['selisih'] !== null
                                 ? (float)$p['selisih']
                                 : ($aktual !== null ? $aktual - $qty : null);
                    $mapeRow = ($aktual !== null && $aktual > 0)
                                 ? abs(($aktual - $qty) / $aktual) * 100
                                 : null;
                    $selColor = $selisih === null ? '' : ($selisih >= 0 ? 'color:var(--accent-green)' : 'color:var(--accent-red)');
                ...
                <tr class="rw-row"
                    ...
                    data-mape="<?= $mapeRow !== null ? $mapeRow : 9999 ?>">
                    ...
                    <td style="text-align:right">
                        <?php if ($aktual !== null): ?>
                            <span class="rw-qty-aktual"><?= number_format($aktual) ?></span>
                        <?php else: ?>
                    ...
                    <td style="text-align:right;<?= $selColor ?>">
                        <?php if ($selisih !== null): ?>
                            <span style="font-weight:600;font-family:'Courier New',monospace;font-size:13px">
                                <i class="bi <?= $selisih >= 0 ? 'bi-arrow-up-short' : 'bi-arrow-down-short' ?>"></i>
                    ...
                    <td style="text-align:center">
                        <?php if ($mapeRow !== null): ?>
                            <span class="mape-pill <?= $mapeRow < 10 ? 'mape-good' : ($mapeRow < 20 ? 'mape-ok' : 'mape-bad') ?>">
                    ...
                    <td style="text-align:center">
                        ...
                        <?php else: ?>
                            <span class="status-pill status-forecast">
    ...
    <div class="card-mg">
        ...
        <div class="rw-produk-grid" id="rwProdukGrid">
            ...
            foreach ($ringkasanProduk as $rank => $rp):
                $mapeAvg  = $rp['mape_avg'] !== null ? (float)$rp['mape_avg'] : null;
                $akurRp   = $mapeAvg !== null ? max(0, 100 - $mapeAvg) : null;
                $barW     = $maxPred > 0 ? min(100, ((float)$rp['total_prediksi'] / $maxPred) * 100) : 0;
                $mapeColor = $mapeAvg === null ? 'var(--text-muted)'
                    : ($mapeAvg < 10 ? 'var(--accent-green)' : ($mapeAvg < 20 ? 'var(--accent-yellow)' : 'var(--accent-red)'));
            ?>
            <div class="rw-produk-card"
                ...
                <div class="rw-pc-header">
                    ...
                    <?php if ($rp['terverifikasi'] > 0): ?>
                        <span class="badge-mg badge-green" style="font-size:10px;padding:1px 7px">
                            <?= $rp['terverifikasi'] ?>× Terverifikasi
                        </span>
                    <?php else: ?>
                ...
                <div class="rw-pc-stats">
                    ...
                    <?php if ($mapeAvg !== null): ?>
                    <div class="rw-pc-stat-item">
                        <span class="rw-pc-stat-label">MAPE Avg</span>
                        <span class="rw-pc-stat-val" style="color:<?= $mapeColor ?>">
                            <?= number_format($mapeAvg, 1) ?>%
                        </span>
                    </div>
                    <?php endif; ?>
                    <?php if ($akurRp !== null): ?>
                    <div class="rw-pc-stat-item">
                        <span class="rw-pc-stat-label">Akurasi</span>
                        <span class="rw-pc-stat-val" style="color:<?= $akurRp >= 80 ? 'var(--accent-green)' : ($akurRp >= 60 ? 'var(--accent-yellow)' : 'var(--accent-red)') ?>">
                            <?= number_format($akurRp, 1) ?>%
                        </span>
                    </div>
                    <?php endif; ?>
                </div>
    ...
    <div class="card-mg">
        <div class="mg-card-header">
            ...
            <div style="display:flex;align-items:center;gap:8px">
                ...
                <span class="rw-legend-item"><span class="rw-leg-dot" style="background:var(--accent-green)"></span>Total Qty Aktual</span>
                <?php endif; ?>
            </div>
        </div>
...
.rw-empty-icon {
    ...
    font-size: 34px; color: var(--text-placeholder);
    margin-bottom: 16px;
}
...
@media(max-width:1100px){ .rw-stats-row{ grid-template-columns: repeat(3,1fr); } }
@media(max-width:700px) { .rw-stats-row{ grid-template-columns: repeat(2,1fr); } }
@media(max-width:420px) { .rw-stats-row{ grid-template-columns: 1fr; } }
...
.rw-stat-val {
    ...
    color: var(--text-ink); line-height: 1.1;
}
...
.mg-search-box input {
    ...
    color: var(--text-ink); font-size: 13px;
    outline: none; width: 150px;
}

.mg-search-box input::placeholder { color: var(--text-placeholder); }

.rw-sort-wrap select { padding:7px 10px; font-size:12.5px; }
...
.rw-produk-icon {
    ...
    border-radius:8px;
    display:flex; align-items:center; justify-content:center;
    color:var(--accent-cyan); font-size:13px; flex-shrink:0;
}

.rw-produk-nama {
    font-size:13px; font-weight:500;
    color:var(--text-ink); line-height:1.3;
}

.rw-periode-badge {
    display:inline-flex; align-items:center;
    padding:2px 10px; border-radius:6px;
    background:rgba(0,151,184,.1); color:var(--accent-cyan);
    font-size:11.5px; font-weight:600;
}

.rw-qty-pred {
    font-family:'Courier New',monospace;
    font-size:14px; font-weight:700; color:var(--mg-light);
}

.rw-qty-aktual {
    font-family:'Courier New',monospace;
    font-size:13px; color:var(--text-ink);
}
...
.mape-good {
    background:rgba(16,183,127,.1); color:var(--accent-green);
    border:1px solid rgba(16,183,127,.2);
}

.mape-ok {
    background:rgba(212,160,23,.1); color:var(--accent-yellow);
    border:1px solid rgba(212,160,23,.2);
}

.mape-bad {
    background:rgba(229,62,62,.1); color:var(--accent-red);
...
.status-forecast {
    background:rgba(74,159,212,.1); color:var(--mg-light);
...
.rw-no-result i { font-size:32px; opacity:.3; }
...
@media(max-width:600px){ .rw-produk-grid{ grid-template-columns: repeat(2,1fr); } }
...
.rw-pc-rank {
    font-size: 11px; font-weight: 700; color: var(--text-placeholder);
    background: var(--card-bg-alt);
    border: 1px solid var(--surface-border);
...
.rw-pc-icon {
    ...
    border-radius: 10px;
    display: flex; align-items: center; justify-content: center;
    color: var(--accent-cyan); font-size: 15px;
}

.rw-pc-nama {
    font-size: 12px; font-weight: 600;
    color: var(--text-ink); line-height: 1.4;
}
...
.rw-pc-stat-val {
    font-size: 13px; font-weight: 700; color: var(--text-ink);
}

/* ── Chart ── */
.rw-chart-wrap { position:relative; padding:4px 0; }
...
.badge-cyan {
    background:rgba(0,151,184,.12); color:var(--accent-cyan);
    border:1px solid rgba(0,151,184,.2);
}

.badge-green {
    background:rgba(16,183,127,.12); color:var(--accent-green);
    border:1px solid rgba(16,183,127,.2);
}

.section-title {
    font-size:14px; font-weight:700; color:var(--text-ink);
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const tablePag = PaginationEngine({
        ...
        onShow:         el => { el.style.display = ''; },
        onHide:         el => { el.style.display = 'none'; },
    ...
    function applySortFilter() {
        ...
        result.sort((a, b) => {
            switch (sortVal) {
                case 'periode_desc': return b.dataset.periode.localeCompare(a.dataset.periode);
                case 'periode_asc':  return a.dataset.periode.localeCompare(b.dataset.periode);
                case 'qty_desc':     return parseFloat(b.dataset.qty)  - parseFloat(a.dataset.qty);
                case 'qty_asc':      return parseFloat(a.dataset.qty)  - parseFloat(b.dataset.qty);
                case 'nama_asc':     return a.dataset.nama.localeCompare(b.dataset.nama);
                case 'mape_asc':     return parseFloat(a.dataset.mape) - parseFloat(b.dataset.mape);
                default: return 0;
            }
        });
    ...
    document.getElementById('btnRwExport')?.addEventListener('click', function () {
        ...
        const headers = ['No','Nama Produk','Periode','Qty Prediksi','Qty Aktual','Selisih','MAPE','Status','Dibuat'];
    ...
    /* ══════════════════════════════════════════════════════════
       5. SINKRON AKTUAL
    ══════════════════════════════════════════════════════════ */
    document.querySelectorAll('.btn-sinkron-aktual').forEach(btn => {
        btn.addEventListener('click', async function () {
            if (typeof Swal !== 'undefined') {
                const r = await Swal.fire({
                    title: 'Sinkronkan Data Aktual?',
                    html:  'Mengisi <b>qty_aktual</b> dari data penjualan<br>untuk semua periode yang tersedia.',
                    icon:  'question',
                    showCancelButton:   true,
                    confirmButtonColor: '#2e6da4',
                    cancelButtonColor:  '#243B55',
                    confirmButtonText:  '<i class="bi bi-arrow-repeat"></i> Ya, Sinkronkan',
            ...
            this.disabled  = true;
            this.innerHTML = '<span class="rw-spin-sm"></span> Menyinkronkan…';

            const daftarPeriode = <?= json_encode($daftarPeriode) ?>;
            let totalUpdated    = 0;

            for (const p of daftarPeriode) {
                const fd = new FormData();
                fd.append('<?= csrf_token() ?>', '<?= csrf_hash() ?>');
                fd.append('tahun', p.tahun);
                fd.append('bulan', p.bulan);
                try {
                    const res  = await fetch(BASE_URL + 'prediksi/sinkron-aktual', {
                        method:  'POST',
                        headers: { 'X-Requested-With': 'XMLHttpRequest' },
                        body:    fd,
                    });
                    const data = await res.json();
                    if (data.status === 'success') totalUpdated += (data.total || 0);
                } catch (e) {
                    console.error('Sinkron gagal untuk periode', p, e);
                }
            }
            ...
            if (typeof Swal !== 'undefined') {
                await Swal.fire({
                    icon:  totalUpdated > 0 ? 'success' : 'info',
                    title: totalUpdated > 0 ? 'Sinkronisasi Selesai' : 'Tidak Ada Perubahan',
                    text:  totalUpdated > 0
                        ? `${totalUpdated} data berhasil diisi qty aktualnya. Halaman akan dimuat ulang.`
                        : 'Tidak ada data penjualan yang cocok untuk semua periode.',
                    confirmButtonColor: '#2e6da4',
                });
            }

            if (totalUpdated > 0) window.location.reload();
        });
    });
    ...
    const periodeData = <?= json_encode(
        (function() use ($prediksiList, $bulanPendek) {
            ...
            foreach ($prediksiList as $p) {
                ...
                if ($p['qty_aktual'] !== null) {
                    $grouped[$key]['aktual']    += (float)$p['qty_aktual'];
                    $grouped[$key]['hasAktual']  = true;
                }
            }
            ksort($grouped);
    ...
    const aktVals   = periodeData.map(d => d.hasAktual ? d.aktual : null);
    const hasAktual = aktVals.some(v => v !== null);
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    display: hasAktual,
                    labels: { color:'#8b9aad', font:{size:12}, boxWidth:12, boxHeight:12 }
                },
                tooltip: {
                    backgroundColor: '#1a2332',
                    borderColor: 'rgba(74,159,212,0.3)', borderWidth: 1,
                    titleColor: '#e2e8f0', bodyColor: '#8b9aad', padding: 12,
                    callbacks: {
            ...
            scales: {
                x: { ticks:{ color:'#5a7a9a', font:{size:11} }, grid:{ color:'rgba(255,255,255,0.04)' } },
